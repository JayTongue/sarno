...
This is modeled as: $I_i(\mathbf{x}) = e^{-\|\mathbf{x} - \mathbf{p}_i\|^2/\sigma^2}$
where 
...
$I_i(\mathbf{x}) = \exp\left(-\frac{\|\mathbf{x} - \mathbf{p}_i\|^2}{9r^2}\right)$
...
In pure form, the integrated equasion is $J = \int_{\Omega} h(\mathbf{x}) \, d\mathbf{x} = \int_{\Omega} \max_{i=1,\ldots,4} e^{-\|\mathbf{x} - \mathbf{p}_i\|^2/(9r^2)} \, d\mathbf{x}$

Assuming two dimentions `Ω = [0, W] x [0, H]`, this can be written as: $J = \int_0^W \int_0^H \max_{i=1,\ldots,4} e^{-\|(\mathbf{x}) - \mathbf{p}_i\|^2/(9r^2)} \, dy \, dx$
